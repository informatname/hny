 ...
 function (){
   BeginInit();
    ...
    P.addEventListener("click", HelpOpen );
   	...
   	setTimeout( DedMoroz, 10000);
}
);

function DivClose(ID){
  var P = document.getElementById(ID);
  P.innerHTML="<IMG src='images/pic.jpg' style='width:0%'/>";
  P.zIndex='-999';
  P.style.width= 0;
  P.style.height= 0;
  StartBalls();
...
function HelpOpen(){
  var P = document.getElementById('HelpScreen');
	...
	P.zIndex='999';
	P.innerHTML="<div style='width:50vw' id='DivInHelp'>"+
...
document.addEventListener("mousemove", 
 ...
 function DedMorozGo(X){
	 var P = document.getElementById('DedMoroz');
	 	
	 P.style.left = X+"px";
     
	 if ( X > -1000)
	 setTimeout( DedMorozGo, 1000/60, Number(X)-10);
 ...
 function DedMoroz(){
    ...
    P.style.top = Math.random()*(window.innerHeight/3)+10+"px";
    
	setTimeout( DedMorozGo, 1000/60, window.innerWidth);
	setTimeout( DedMoroz, 10000*(3*Math.random()+1));
}
